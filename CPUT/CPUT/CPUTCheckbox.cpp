...
// Constructor
//-----------------------------------------------------------------------------
CPUTCheckbox::CPUTCheckbox(const cString ControlText, CPUTControlID id, CPUTFont *pFont, float scale) :
    mbMouseInside(false),
...
void CPUTCheckbox::CalculateBounds()
{
    ...
    {
        mControlDimensions.height = mpCheckboxTextureSizeList[0].height * mScale;
    }
}
...
void CPUTCheckbox::GetTextPosition(int &x, int &y)
{
    ...
    x = mControlDimensions.x + mpCheckboxTextureSizeList[0].width * mScale  + CPUT_CHECKBOX_PADDING; // move right far enough not to overlap the bitmap
    y = mControlDimensions.y;
}
...
void CPUTCheckbox::Recalculate()
{
    ...
    AddQuadIntoMirrorBuffer(mpMirrorBufferActive, xPos, yPos, width, height,
                            mpUVCoordsCheckbox_active[0], mpUVCoordsCheckbox_active[3]
                        );

    // pressed
    AddQuadIntoMirrorBuffer(mpMirrorBufferPressed, xPos, yPos, width, height,
                            mpUVCoordsCheckbox_pressed[0], mpUVCoordsCheckbox_pressed[3]
                        );

    // disabled
    AddQuadIntoMirrorBuffer(mpMirrorBufferDisabled, xPos, yPos, width, height,
