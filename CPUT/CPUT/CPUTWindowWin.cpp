...
LRESULT CALLBACK CPUTWindowWin::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        if(mCPUT)
        {
            ...
            CPUTEventID cputmousemessage = CPUT_EVENT_MOVE;
            if(message == WM_LBUTTONDOWN || message == WM_MBUTTONDOWN || message == WM_RBUTTONDOWN)
            {
                cputmousemessage = CPUT_EVENT_DOWN;
            }
            else if(message == WM_LBUTTONUP || message == WM_MBUTTONUP || message == WM_RBUTTONUP)
            {
                cputmousemessage = CPUT_EVENT_UP;
            }
            handledCode = mCPUT->CPUTHandleMouseEvent(xPos, yPos, 0, state, cputmousemessage);
        }
        break;
