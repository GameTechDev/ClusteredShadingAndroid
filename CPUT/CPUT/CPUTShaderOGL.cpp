    ...
    )
{
    ...
    cString name = CPUTShaderOGL::CreateShaderName(fileNames, shaderType, pShaderMacros);
    pAssetLibrary->AddPixelShader(name, pNewCPUTShader);
...
cString CPUTShaderOGL::CreateShaderName(const std::vector<cString> &fileNames,
                                            GLuint shaderType,
                                            CPUT_SHADER_MACRO *pShaderMacros)
{
    
    cString libName(to_cString(shaderType)); 
